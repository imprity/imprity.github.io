...
function arrayRemove<T>(array: Array<T>, toRemove: number): Array<T> {
...
function arrayRemoveFast<T>(array: Array<any>, toRemove: number): Array<T> {
    if (!(0 <= toRemove && toRemove < array.length)) {
        return array
    }
...
function fuzzyMatch(str: string, sub: string): {
    start: number,
    length: number
    distance: number
} {
...
function blurItAndChildren(element: HTMLElement) {
